...
fn rle_encode_u32(bencher: Bencher) {
    ...
    bencher.bench_local(|| {
        ...
        let unique_count =
            unsafe { u32::encode_unchecked(black_box(&input.0), &mut rle_vals.0, &mut rle_idxs.0) };
...
fn rle_decode_u32(bencher: Bencher) {
    ...
    let mut rle_idxs = Aligned([0u16; 1024]);
    // SAFETY: all arguments are 1024-element arrays.
    let unique_count = unsafe { u32::encode_unchecked(&input.0, &mut rle_vals.0, &mut rle_idxs.0) };
    ...
    with_counter!(bencher, input_data.len() * std::mem::size_of::<u32>()).bench_local(|| {
        ...
        for batch in 0..NUM_BATCHES {
            ...
            let unique_count = unsafe {
                u32::encode_unchecked(black_box(input_batch), &mut rle_vals.0, &mut rle_idxs.0)
            };
            black_box(unique_count);
        }
    });
...
fn rle_throughput_decode_32(bencher: Bencher) {
    ...
    for batch in 0..NUM_BATCHES {
        ...
        // SAFETY: all arguments are 1024-element arrays.
        let unique_count =
            unsafe { u32::encode_unchecked(&input_batch, &mut rle_vals.0, &mut rle_idxs.0) };
