use std::mem::size_of;

use arrayref::{array_mut_ref, array_ref};
use divan::{counter::BytesCount, Bencher};
use fastlanes::{BitPacking, FastLanes};
use std::hint::black_box;

fn main() {
    divan::main();
}

#[divan::bench]
fn pack_16_to_3_heap(bencher: Bencher) {
    const WIDTH: usize = 3;
    const B: usize = 1024 * WIDTH / u16::T;

    bencher
        .with_inputs(|| {
            let values = vec![3u16; 1024];
            let packed = vec![0; 128 * WIDTH / size_of::<u16>()];
            (values, packed)
        })
        .bench_local_refs(|(values, packed)| {
            BitPacking::pack::<WIDTH, B>(
                black_box(array_ref![values, 0, 1024]),
                array_mut_ref![packed, 0, 192],
            );
            black_box(packed);
        });
}

#[divan::bench]
fn pack_16_to_3_stack(bencher: Bencher) {
    const WIDTH: usize = 3;
    const B: usize = 1024 * WIDTH / u16::T;

    bencher
        .with_inputs(|| {
            let values = [3u16; 1024];
            let packed = [0; 128 * WIDTH / size_of::<u16>()];
            (values, packed)
        })
        .bench_local_refs(|(values, packed)| {
            BitPacking::pack::<WIDTH, B>(black_box(values), packed);
            black_box(*packed);
        });
}

#[divan::bench]
fn unpack_16_from_3_stack(bencher: Bencher) {
    const WIDTH: usize = 3;
    const B: usize = 1024 * WIDTH / u16::T;

    bencher
        .with_inputs(|| {
            let values = [3u16; 1024];
            let mut packed = [0; 128 * WIDTH / size_of::<u16>()];
            BitPacking::pack::<WIDTH, B>(&values, &mut packed);
            let unpacked = [0u16; 1024];
            (packed, unpacked)
        })
        .bench_local_refs(|(packed, unpacked)| {
            BitPacking::unpack::<WIDTH, B>(black_box(packed), unpacked);
            black_box(*unpacked);
        });
}

#[divan::bench]
fn unchecked_unpack_16_from_3_stack(bencher: Bencher) {
    const WIDTH: usize = 3;
    const B: usize = 1024 * WIDTH / u16::T;

    bencher
        .with_inputs(|| {
            let values = [3u16; 1024];
            let mut packed = [0; 128 * WIDTH / size_of::<u16>()];
            BitPacking::pack::<WIDTH, B>(&values, &mut packed);
            let unpacked = [0u16; 1024];
            (packed, unpacked)
        })
        .bench_local_refs(|(packed, unpacked)| {
            unsafe {
                BitPacking::unchecked_unpack(WIDTH, black_box(packed), unpacked);
            };
            black_box(*unpacked);
        });
}

#[divan::bench]
fn unpack_single_16_from_3(bencher: Bencher) {
    const WIDTH: usize = 3;
    const B: usize = 1024 * WIDTH / u16::T;

    bencher
        .with_inputs(|| {
            let values = vec![3u16; 1024];
            let mut packed = vec![0; 128 * WIDTH / size_of::<u16>()];
            BitPacking::pack::<WIDTH, B>(
                array_ref![values, 0, 1024],
                array_mut_ref![packed, 0, 192],
            );
            packed
        })
        .bench_local_refs(|packed| {
            for i in 0..1024 {
                black_box::<u16>(BitPacking::unpack_single::<WIDTH, B>(
                    black_box(array_ref![packed, 0, 192]),
                    black_box(i),
                ));
            }
        });
}

#[divan::bench]
fn throughput_compress(bencher: Bencher) {
    const WIDTH: usize = 3;
    const B: usize = 1024 * WIDTH / u16::T;
    const NUM_BATCHES: usize = 1024;
    const N: usize = 1024 * NUM_BATCHES;
    const OUTPUT_BATCH_SIZE: usize = 128 * WIDTH / size_of::<u16>();

    bencher
        .counter(BytesCount::new(N * size_of::<u16>()))
        .with_inputs(|| {
            let values: Vec<u16> = (0..N).map(|i| (i % 8) as u16).collect();
            let packed = vec![0u16; NUM_BATCHES * OUTPUT_BATCH_SIZE];
            (values, packed)
        })
        .bench_local_refs(|(values, packed)| {
            for i in 0..NUM_BATCHES {
                BitPacking::pack::<WIDTH, B>(
                    black_box(array_ref![values, i * 1024, 1024]),
                    array_mut_ref![packed, i * OUTPUT_BATCH_SIZE, OUTPUT_BATCH_SIZE],
                );
            }
            black_box(packed);
        });
}

#[divan::bench]
fn throughput_decompress(bencher: Bencher) {
    const WIDTH: usize = 3;
    const B: usize = 1024 * WIDTH / u16::T;
    const NUM_BATCHES: usize = 1024;
    const N: usize = 1024 * NUM_BATCHES;
    const OUTPUT_BATCH_SIZE: usize = 128 * WIDTH / size_of::<u16>();

    bencher
        .counter(BytesCount::new(N * size_of::<u16>()))
        .with_inputs(|| {
            let values: Vec<u16> = (0..N).map(|i| (i % 8) as u16).collect();
            let mut packed = vec![0u16; NUM_BATCHES * OUTPUT_BATCH_SIZE];
            // Pre-pack the data for decompression benchmark
            for i in 0..NUM_BATCHES {
                BitPacking::pack::<WIDTH, B>(
                    array_ref![values, i * 1024, 1024],
                    array_mut_ref![packed, i * OUTPUT_BATCH_SIZE, OUTPUT_BATCH_SIZE],
                );
            }
            let output_values = vec![0u16; N];
            (packed, output_values)
        })
        .bench_local_refs(|(packed, values)| {
            for i in 0..NUM_BATCHES {
                BitPacking::unpack::<WIDTH, B>(
                    black_box(array_ref![packed, i * OUTPUT_BATCH_SIZE, OUTPUT_BATCH_SIZE]),
                    array_mut_ref![values, i * 1024, 1024],
                );
            }
            black_box(values);
        });
}
